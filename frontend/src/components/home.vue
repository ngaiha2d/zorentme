...
const itemsPerPage = 11; // Update the items per page value here
...
onMounted(async () => {
  ...
  items.value = store.item;
});

function page(itemId) {
  store.loadPage(itemId);
}
...
<template>
  ...
  <div class="card-container">
    <div class="card" v-for="(item, id) in paginatedItems" :key="id" @click="page(item.id)">
      <div class="card-img">
        <img class="imageTo" :src="'http://localhost:8000/storage/' + item.file_path.replace('public/', '')" alt="image">  
      </div>
      <div class="card-title">{{ item.name }}</div>
      <div class="card-subtitle">{{ item.description }}</div>
      <hr class="card-divider">
      <div class="card-footer">
        <div class="card-price"><span>$</span>{{ item.price }}  <span>{{ item.rent_time }}</span> day</div>
      </div>
