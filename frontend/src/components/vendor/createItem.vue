...
const user_id = ref(authStore.authUser.id);
...
async function formSubmit() {
  try {
    ...
    formData.append('rent_time', '1');
    formData.append('user_id', form.value.user_id);
    console.log(...formData);
    // Make the API request
    const response = await axios.post('/api/all/item', formData, {
      headers: {
...
<div class="form-container">
  ...
  <label>Text</label>
  <input type="text" v-model="form.name" name="name" placeholder="Item Name" />
</div>
...
<div class="nice-form-group">
  ...
  <select v-model="form.categories" name="categories">
    <option>Please select Categories</option>
    <option>2_Wheeler</option>
    <option>4_Wheeler</option>
  </select>
</div>
...
<div class="nice-form-group">
  ...
  <input type="text" v-model="form.price" name="price" placeholder="Rs. 300 /-" />
</div>
